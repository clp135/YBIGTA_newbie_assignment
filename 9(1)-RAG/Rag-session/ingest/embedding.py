...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    with ThreadPoolExecutor(max_workers=num_keys) as executor:
        # 각 클라이언트(키)에게 작업을 분배
        futures = {
            executor.submit(_embed_batch_safe, clients[idx % num_keys], chunk): (idx, chunk_start)
            for idx, (chunk_start, chunk) in enumerate(chunks)
        }

        done_count = 0
        for future in as_completed(futures):
            _, chunk_start = futures[future]
            batch_res = future.result()
            # 원래 위치에 결과 저장
            all_embeddings[chunk_start : chunk_start + len(batch_res)] = batch_res
            done_count += len(batch_res)
